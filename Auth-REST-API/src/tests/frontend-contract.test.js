...
describe("Frontend API contract", () => {
  it("keeps auth payload shape used by Auth-FrontEnd", async () => {
    ...
    assert.equal(typeof signup.body.expiresIn, "string");
    assert.equal(typeof signup.body.user.id, "string");
    assert.equal(typeof signup.body.user.email, "string");
    assert.equal(typeof signup.body.user.name, "string");
