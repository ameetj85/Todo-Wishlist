...
async function signupAndVerify(user = VALID_USER) {
  const signup = await request(app, 'POST', '/api/auth/signup', user);
  assert.equal(signup.status, 201);
  assert.equal(signup.body.requiresVerification, true);

  const createdUser = await getUser(user.email);
...
async function createVerifiedSession(user = VALID_USER) {
...
describe('POST /api/auth/signup', () => {
  it('creates account and requires verification', async () => {
...
describe('POST /api/auth/logout-all', () => {
  it('invalidates all sessions', async () => {
    const token1 = await createVerifiedSession();
    const s2 = await request(app, 'POST', '/api/auth/login', {
      email: VALID_USER.email,
      password: VALID_USER.password,
    });
    assert.equal(s2.status, 200);
    assert.ok(s2.body.token);
